...
@celery.task()
def create_reservation(request, user, uuid=None):
    reservation_info = {}
    ...
    try:
        ...
        # Write request into json
        reservation_info.update({"request":request})
        ...
        if reservation_info.get("error") == None:
            ...
            print(reservation_info.get("state"))
            if reservation_info.get("state") == "IN PROGRESS":
                state = state_json("ERROR")
            else:
                state = {"state":reservation_info.get("state")}
